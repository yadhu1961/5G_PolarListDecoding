...
//    bool _llr_based_computation;
    ...
    void LLR_quantizer(std::vector<double> &input) {

        int number_of_levels = pow(2,8);

        for(unsigned i = 0; i < _block_length;i++) {

            input[i] = floor(number_of_levels / 2 * tanh(-input[i] / 2));// + number_of_levels/2;

//            if(input[i] >= number_of_levels)
//                input[i] = number_of_levels - 1;
//
//            if(input[i] < 0 )
//                input[i] = 0;
        }
